...
| Cynefin Map | Domain Mix, Evidence, Confidence 기반 해석 기록 |
...
Story 입력이 막히면 **AI Assist**를 누릅니다.
...
AI Assist는 사용자가 입력한 Title과 Story 본문을 바탕으로 초안을 제안합니다. 제안은 확정값이 아니며, 사용자가 검토한 뒤 **Add Story**를 눌러 저장합니다.
...
### 5.2 3-step Domain Mapping

Cynefin Map은 3단계로 구성됩니다.

#### Step 1. Domain Mix

Clear, Complicated, Complex, Chaotic, Aporetic 슬라이더를 조정합니다.

이 값은 이슈가 각 도메인에 얼마나 가까워 보이는지에 대한 **해석 분포**입니다. 도넛 차트와 퍼센트 분포가 자동으로 바뀌고, 가장 높은 값은 Dominant Domain 후보로 표시됩니다.

#### Step 2. Why This Interpretation?

Evidence Story를 선택하고 Rationale을 입력합니다.

입력할 것:
- Evidence Story: 이 해석의 근거가 되는 Story
- Rationale: 왜 현재 Domain Mix로 보는지에 대한 설명

**AI Assist**를 누르면 현재 Domain Mix와 Evidence Story를 바탕으로 Rationale 초안, 검토 질문, caution을 제안합니다.

#### Step 3. How Sure Are You?

Confidence를 1~5로 기록합니다.

중요: Confidence는 Domain Mix 수치를 바꾸지 않습니다. Confidence는 현재 해석에 대한 확신도입니다.

예:
- Complex 70%, Complicated 20%, Aporetic 10%, Confidence 2/5  
  → Complex로 보이지만 확신은 낮음
- Complex 70%, Complicated 20%, Aporetic 10%, Confidence 5/5  
  → Complex 해석에 높은 확신이 있음

마지막으로 **Save Interpretation**을 누르면 Interpretation Log에 저장됩니다.
...
### 5.4 Interpretation Log

Interpretation Log는 저장된 해석 기록입니다.
...
해석 로그는 나중에 팀이 어떻게 판단이 바뀌었는지 돌아보는 데 사용합니다.
...
Probe 설계가 막히면 **AI Assist**를 누릅니다.
...
- Success Signal
- Failure Signal
- Safe-to-Fail 범위 점검 질문

제안값은 입력 폼에 자동으로 채워집니다. 검토 후 **Add Probe**를 눌러 저장합니다.
...
### 7.2 Signal AI Assist

Signal 기록이 막히면 **AI Assist**를 누릅니다.
...
제안값은 입력 폼에 자동으로 채워집니다. 검토 후 **Add Signal**을 눌러 저장합니다.
...
### Confidence를 바꿨는데 Domain Mix가 바뀌지 않아요

정상입니다. Confidence는 도메인 비율을 바꾸지 않습니다. 현재 Domain Mix 해석에 대한 확신도를 기록하는 값입니다.
